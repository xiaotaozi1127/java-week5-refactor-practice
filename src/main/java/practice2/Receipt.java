...
public class Receipt {
    ...
    public double CalculateGrandTotal(List<Product> products, List<OrderItem> items) {
        ...
        subTotal = updateTotalByDiscount(products, items, subTotal);
    ...
    private BigDecimal updateTotalByDiscount(List<Product> products, List<OrderItem> items, BigDecimal subTotal) {
        for (Product product : products) {
            OrderItem curItem = findOrderItemByProduct(items, product);
            BigDecimal reducedPrice = getProductReducedPrice(product, curItem);
            subTotal = subTotal.subtract(reducedPrice);
        }
        return subTotal;
    }

    private BigDecimal getProductReducedPrice(Product product, OrderItem curItem) {
    ...
    private BigDecimal calculateSubtotal(List<Product> products, List<OrderItem> items) {
        ...
        for (Product product : products) {
            ...
            BigDecimal itemTotal = getProductsPrice(product, item);
            subTotal = subTotal.add(itemTotal);
        }
        return subTotal;
    }

    private BigDecimal getProductsPrice(Product product, OrderItem item) {
